...
async function nginx() {
	return `
		...
		<head>
		    ...
		    <title>Public Domain Name Registration Service | DNSHE 公共域名注册服务</title>
		    <style>
		        :root{--primary:#10b981;--primary-hover:#059669;--link-color:#2563eb;--text-main:#0f172a;--text-muted:#334155;--text-desc:#475569;--danger-bg:rgba(239,68,68,.06);--danger-border:rgba(239,68,68,.2);--danger-text:#dc2626}
		        *{box-sizing:border-box;margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,"SF Pro Text","SF Pro Display","Helvetica Neue",Arial,sans-serif;-webkit-font-smoothing:antialiased}
		        body{background:#f1f5f9;background:radial-gradient(circle at 80% 20%,#e0f2fe 0,transparent 40%),radial-gradient(circle at 20% 80%,#f0fdf4 0,transparent 50%),linear-gradient(180deg,#f8fafc 0,#e2e8f0 100%);background-attachment:fixed;min-height:100vh;display:flex;align-items:flex-start;justify-content:center;padding:0 24px;color:var(--text-main);overflow-y:auto}
		        .container{background:rgba(255,255,255,.5);backdrop-filter:blur(30px) saturate(160%);-webkit-backdrop-filter:blur(30px) saturate(160%);border:1px solid rgba(255,255,255,.7);border-radius:28px;padding:30px 48px 36px;width:100%;max-width:780px;box-shadow:0 4px 30px rgba(0,0,0,.01),0 20px 50px rgba(15,23,42,.05);position:relative;margin-top:3.5vh;margin-bottom:24px}
		        .header-bar{display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;padding-bottom:14px;border-bottom:1px solid rgba(15,23,42,.04)}
		        .logo-text{font-size:38px;font-weight:900;letter-spacing:-1.5px;background:linear-gradient(135deg,#2563eb 0,#10b981 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
		        .lang-switcher{background:rgba(15,23,42,.05);padding:3px;border-radius:12px;display:inline-flex;gap:2px;border:1px solid rgba(255,255,255,.5)}
		        .lang-btn{padding:5px 16px;border-radius:9px;font-size:14px;font-weight:700;color:var(--text-desc);cursor:pointer;transition:all .2s ease;border:none;background:transparent}
		        .lang-btn.active{background:#fff;color:var(--text-main);box-shadow:0 2px 6px rgba(15,23,42,.06)}
		        .domain-tag-wrap{text-align:center;margin-bottom:16px}
		        .current-domain{display:inline-block;background:rgba(255,255,255,.85);border:1.5px solid rgba(16,185,129,.25);color:var(--text-muted);padding:8px 28px;border-radius:99px;font-weight:600;font-size:16.5px;letter-spacing:.5px;box-shadow:0 4px 14px rgba(16,185,129,.06)}
		        #domain-span{text-transform:lowercase;color:var(--primary);font-weight:800;font-size:22px;margin-left:2px}
		        h1{font-size:29px;font-weight:800;margin-bottom:14px;color:var(--text-main);text-align:center;letter-spacing:-.5px}
		        .notice-text{font-size:17.5px;line-height:1.65;color:var(--text-muted);margin-bottom:18px;text-align:left;background:rgba(255,255,255,.25);padding:18px 24px;border-radius:16px;border:1px solid rgba(255,255,255,.3)}
		        .notice-text p{margin-bottom:6px}.notice-text p:last-child{margin-bottom:0}
		        .platform-link{color:var(--text-main);text-decoration:underline;font-weight:700;transition:color .2s ease}
		        .platform-link:hover{color:var(--primary)}
		        .abuse-section{text-align:left;background:var(--danger-bg);border:1px solid var(--danger-border);border-radius:16px;padding:20px 24px;margin-bottom:20px}
		        .abuse-title{font-size:18px;font-weight:800;color:var(--danger-text);margin-bottom:14px;display:flex;align-items:center;gap:8px}
		        .abuse-item{font-size:17px;color:var(--text-muted);margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;background:rgba(255,255,255,.7);padding:11px 16px;border-radius:12px;border:1px solid rgba(255,255,255,.5)}
		        .abuse-item:last-child{margin-bottom:0}
		        .abuse-item a{color:var(--text-main);text-decoration:none;font-weight:700}
		        .abuse-item a:hover{color:var(--link-color);text-decoration:underline}
		        .abuse-tip{font-size:14.5px;color:#991b1b;margin-top:10px;line-height:1.5;opacity:.95;text-align:left;border-top:1px dashed rgba(239,68,68,.15);padding-top:8px}
		        .btn-group{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:0}
		        .btn{display:flex;align-items:center;justify-content:center;padding:14px 24px;border-radius:14px;font-size:17.5px;font-weight:700;text-decoration:none;transition:all .2s ease}
		        .btn-primary{background:linear-gradient(135deg,#10b981 0,#059669 100%);color:#fff;box-shadow:0 4px 14px rgba(16,185,129,.2)}
		        .btn-primary:hover{transform:translateY(-1px);box-shadow:0 6px 18px rgba(16,185,129,.25)}
		        .btn-secondary{background:rgba(255,255,255,.8);color:var(--text-main);border:1px solid rgba(15,23,42,.08)}
		        .btn-secondary:hover{background:#fff;transform:translateY(-1px);box-shadow:0 4px 14px rgba(15,23,42,.04)}
		        .footer{margin-top:22px;font-size:13px;color:#94a3b8;text-align:center}
		...
		<body>
			<div class="container">
			    <div class="header-bar">
			        <div class="logo-area"><span class="logo-text">DNSHE</span></div>
			        <div class="lang-switcher-wrap">
			            <div class="lang-switcher">
			                <button id="btn-en" class="lang-btn active" onclick="changeLang('en')">English</button>
			                <button id="btn-zh" class="lang-btn" onclick="changeLang('zh')">简体中文</button>
			            </div>
			        </div>
			    </div>
			    <div class="domain-tag-wrap">
			        <span class="current-domain">
			            <span id="prefix-en">Current Public Domain:</span>
			            <span id="prefix-zh" style="display:none;">当前公共域名:</span>
			            <span id="domain-span">loading...</span>
			        </span>
			    </div>
			    <h1 class="lang-en">Public Domain Registration Service</h1>
			    <div class="notice-text lang-en">
			        <p>This root domain is managed by the <a href="https://www.dnshe.com" target="_blank" class="platform-link">DNSHE Free Domain Service Platform</a>. Users are permitted to register and operate independent subdomains under the hosting namespace provided by DNSHE.</p>
			        <p>DNSHE solely provides DNS resolution and basic technical hosting services. The content, purpose, and operations of each subdomain are the exclusive responsibility of its respective owner. DNSHE assumes no legal liability for user-generated content.</p>
			    </div>
			    <h1 class="lang-zh">公共域名注册服务说明</h1>
			    <div class="notice-text lang-zh">
			        <p>此根域名由 <a href="https://www.dnshe.com" target="_blank" class="platform-link">DNSHE 免费域名服务平台</a> 托管。用户可在 DNSHE 提供的命名空间下，自由注册并运行独立的子域名。</p>
			        <p>DNSHE 仅提供 DNS 解析与基础技术托管服务。各子域名的内容与运营行为由所有者自行负责，平台不承担任何因用户生成内容引发的法律责任。</p>
			    </div>
			    <div class="abuse-section">
			        <div class="abuse-title">
			            ...
			            <span class="lang-en">🚨 Abuse & Compliance Report</span>
			            <span class="lang-zh">🚨 滥用与违规举报通道</span>
			        </div>
			        <div class="abuse-item">
			            <div><span class="lang-en">Abuse Email: </span><span class="lang-zh">投诉邮箱：</span></div>
			            <span id="secure-email"></span>
			        </div>
			        <div class="abuse-item">
			            <div><span class="lang-en">Online Report: </span><span class="lang-zh">在线举报：</span></div>
			            <a href="https://www.dnshe.com/domainabuse/" target="_blank" style="color:var(--link-color);"><span class="lang-en">Report Portal &rarr;</span><span class="lang-zh">进入快捷举报通道 &rarr;</span></a>
			        </div>
			        <div class="abuse-tip">
			            <span class="lang-en"><strong>Important Notice:</strong> The platform strictly prohibits any abuse, fraud, pornography, infringement, virus distribution, or any other illegal activities. Upon verification, the platform will immediately take compliance measures, including banning accounts, canceling, or deleting domains.</span>
			            <span class="lang-zh"><strong>重要提示：</strong>平台严禁任何滥用、欺诈、色情、侵权、分发病毒等违法违规行为。一经核实，平台将立即采取封禁账户、注销/删除域名等合规治理措施。</span>
			        </div>
			    </div>
			    <div class="btn-group">
			        <a href="https://www.dnshe.com" class="btn btn-primary" target="_blank"><span class="lang-en">Register Free Subdomain</span><span class="lang-zh">注册免费子域名</span></a>
			        <a href="https://my.dnshe.com/announcements/10/DNSHE-Domain-Name-Service-Terms-of-Service.html" class="btn btn-secondary" target="_blank"><span class="lang-en">Terms of Service (TOS)</span><span class="lang-zh">服务条款 (TOS)</span></a>
			    </div>
			    <div class="footer">&copy; <script>document.write(new Date().getFullYear())</script> DNSHE. All rights reserved.</div>
			</div>
			<script>
			    (function initDomainDisplay() {
			        const hostname = window.location.hostname || '47.128.150.127';
			        document.getElementById('domain-span').innerText = hostname;
			        const isIP = /^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/.test(hostname) || hostname.includes(':');
			        if (isIP) {
			            document.getElementById('prefix-en').innerText = "Current Node IP:";
			            document.getElementById('prefix-zh').innerText = "当前节点 IP：";
			        }
			    })();
			    (function renderSecureEmail() {
			        const u = "abuse", d = "dnshe.com", el = document.getElementById('secure-email');
			        el.innerHTML = `<a href="mailto:${u}@${d}" style="color: var(--danger-text); font-weight:700;">${u}@${d}</a>`;
			    })();
			    function changeLang(lang) {
			        const zh = document.querySelectorAll('.lang-zh'), en = document.querySelectorAll('.lang-en');
			        const btnZh = document.getElementById('btn-zh'), btnEn = document.getElementById('btn-en');
			        const pZh = document.getElementById('prefix-zh'), pEn = document.getElementById('prefix-en');
			        if (lang === 'zh') {
			            en.forEach(e=>e.style.display='none');
			            zh.forEach(e=>e.style.display='block');
			            document.querySelectorAll('span.lang-zh').forEach(e=>e.style.display='inline');
			            btnEn.classList.remove('active'); btnZh.classList.add('active');
			            pEn.style.display='none'; pZh.style.display='inline';
			        } else {
			            zh.forEach(e=>e.style.display='none');
			            en.forEach(e=>e.style.display='block');
			            document.querySelectorAll('span.lang-en').forEach(e=>e.style.display='inline');
			            btnZh.classList.remove('active'); btnEn.classList.add('active');
			            pZh.style.display='none'; pEn.style.display='inline';
			        }
			    }
			</script>
		</body>
